...
const MessageScreen = ({ route }) => {
  ...
  return (
    <View>
      <Surface
        style={{ padding: 10, flexDirection: "row", justifyContent: "space-between", borderBottomLeftRadius: 15, borderBottomRightRadius: 15 }}
      >
        <Button style={{ marginLeft: 15 }} compact icon="new-box" mode="outlined" onPress={() => setSortOrder("newest")}>
          Newest
        </Button>
        <Button compact icon="history" mode="outlined" onPress={() => setSortOrder("oldest")}>
          Oldest
        </Button>
        <Button style={{ marginRight: 15 }} compact icon="comment-plus" mode="outlined" onPress={() => setSortOrder("mostReplies")}>
          Most Replies
        </Button>
      </Surface>
